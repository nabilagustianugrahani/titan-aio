...
async def test_audit_query():
    logger = AuditLogger()
    # Use unique action to avoid DB state leakage from previous tests
    import uuid
    action = f"test.action.{uuid.uuid4().hex[:8]}"
    await logger.log(action=action, actor="test_user")
    await logger.log(action=action, actor="test_user")
    entries = await logger.query(action=action)
...
async def test_revert_version():
    ...
    await mgr.create_version("hook", "hook-1", "Changed")
