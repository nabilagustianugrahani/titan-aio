...
class TestAnalyzeCompetitors:
    ...
    async def test_competitors(self):
        ...
        assert result.competitors_analyzed >= 0  # bisa 0 kalo Shopee API down
        # Hooks bisa kosong kalo LLM parsing gagal — itu ok
        assert isinstance(result.winning_hooks, list)
...
class TestGenerateOffer:
    ...
    async def test_generate_offer(self):
        from MCP.schemas import AnalyzeProductOutput, GenerateOfferInput
...
class TestGenerateScript:
    ...
    async def test_generate_script(self):
        from MCP.schemas import GenerateOfferOutput, GenerateScriptInput, Hook
