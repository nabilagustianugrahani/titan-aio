...
# ── Context Compaction (Forge-inspired) ──────────────────────────

class CompactionStrategy(str, Enum):
    """Context compaction strategies."""

    NO_COMPACT = "no_compact"
    TIERED_COMPACT = "tiered_compact"  # keep last N, summarize older
    SLIDING_WINDOW = "sliding_window"  # keep last N steps only


class ContextCompactor:
    """Manages pipeline context to prevent unbounded growth.

    Patterns inspired by Forge's compaction strategies:
    - NO_COMPACT: keep everything (for short pipelines)
    - TIERED_COMPACT: keep last N steps verbatim, summarize older ones
    - SLIDING_WINDOW: only keep last N steps, drop older

    Tracks total estimated token usage and triggers compaction at threshold.
    """

    def __init__(
        self,
        strategy: CompactionStrategy = CompactionStrategy.TIERED_COMPACT,
        verbatim_steps: int = 5,
        token_threshold: int = 50_000,  # estimated tokens before compaction
    ):
        self.strategy = strategy
        self.verbatim_steps = verbatim_steps
        self.token_threshold = token_threshold
        self._compacted_steps: list[str] = []
    ...
    def should_compact(self) -> bool:
        """Check if context has grown past the threshold."""
        return self._estimated_tokens > self.token_threshold

    def record_step(self, step_name: str, step_output: Any) -> str:
        """Record a step and return its summary.

        Returns full text or summarized text depending on strategy and depth.
        """
        raw = str(step_output) if step_output else ""
        self._estimated_tokens += self._estimate_tokens(raw)

        if self.strategy == CompactionStrategy.NO_COMPACT:
            self._compacted_steps.append(raw)
            return raw

        if self.strategy == CompactionStrategy.SLIDING_WINDOW:
            self._compacted_steps.append(raw)
            if len(self._compacted_steps) > self.verbatim_steps:
                self._compacted_steps = self._compacted_steps[-self.verbatim_steps:]
            return raw

        if self.strategy == CompactionStrategy.TIERED_COMPACT:
            depth = len(self._compacted_steps)
            if depth < self.verbatim_steps:
                # Keep verbatim
                self._compacted_steps.append(raw)
                return raw
            # Summarize by truncating to first 200 chars
            summary = raw[:200] + "…" if len(raw) > 200 else raw
            self._compacted_steps.append(f"[compacted] {step_name}: {summary}")
    ...
    def get_compacted_context(self) -> str:
        """Get the full compacted context as a single string."""
        return "\n".join(self._compacted_steps)
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "steps_recorded": len(self._compacted_steps),
            "compact_count": self._compact_count,
            "verbatim_steps": self.verbatim_steps,
        }
        ...
        if result is not None and validator is not None:
            try:
                if not validator(result):
                    last_error = "Validation failed: output did not pass validator"
