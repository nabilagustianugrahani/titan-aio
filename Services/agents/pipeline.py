...
class Pipeline:
    ...
    async def run(self, url: str, **kwargs) -> SharedState:
        ...
        Agents communicate via MessageBus — each agent publishes its results
        so downstream or reactive agents can listen and act without direct coupling.
        ...
        # MemoryAgent auto-learns from content events
        if "memory" in self.agents:
            self.bus.subscribe("content.generated", self._on_content_generated)
        ...
        for attempt in range(retries + 1):
            try:
                result = await asyncio.wait_for(
                    agent(state=state, bus=self.bus, **kwargs),
                    timeout=tout,
                )
    ...
    # ── Bus Event Handlers (Reactive Agents) ──────────────────────────

    async def _on_content_generated(self, event: dict) -> None:
        """Reactive handler: when content is generated, MemoryAgent auto-learns."""
        data = event.get("data", {})
        product_id = data.get("product_id", "")
        if not product_id or "memory" not in self.agents:
            return
        try:
            ...
            await memory(
                ...
                action="learn",
                category="hooks",
                pattern=f"Content generated for {product_id}",
                evidence=[],
                advice="Auto-learned from content.generated event",
                confidence=0.3,
            )
        except Exception as e:
            logger.debug("Reactive memory agent skipped: %s", e)
