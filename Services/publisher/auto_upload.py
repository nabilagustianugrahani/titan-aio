"""AutoUploader -- BrowserUse-based social media upload agent + Zernio API.

Two posting modes (Zernio preferred):
1. **Zernio API** — REST-based posting, no browser needed (all platforms).
   Requires ``ZERNIO_API_KEY`` in env.
2. **BrowserUse** — Playwright browser automation fallback.
"""
...
class AutoUploader:
    ...
    async def _get_zernio_client(self, platform: str = ""):  # type: ignore[no-untyped-def]
        """Lazy-load the right Zernio client for *platform*."""
        from Services.api.zernio_client import ZernioClient
        from titan.config import settings

        key = settings.zernio_api_key_for(platform)
        if not key:
            return None
        return ZernioClient(api_key=key)

    async def _upload_via_zernio(
        self,
        platform: str,
        caption: str,
        ...
        *,
        ...
        """Upload via Zernio API (preferred path).

        Steps:
        1. If ``media_file`` is given, upload it to Zernio storage first.
        2. Auto-find the first connected account for *platform*.
        3. Create and publish the post via Zernio's single API call.

        Returns a dict mirroring the BrowserUse return shape.
        """
        zernio = await self._get_zernio_client()
        if not zernio:
            return self._error(platform, "ZERNIO_API_KEY not configured")

        if not zernio.is_authenticated:
            return self._error(platform, "ZERNIO_API_KEY invalid or missing")

        # 1. Resolve media URL --------------------------------------------------
        public_url = media_url
        if not public_url and media_file:
            resolved = self._resolve_video(media_file)
            if resolved:
                # For small files (< 25MB), use upload-direct; otherwise presign
                import os
                fsize = os.path.getsize(resolved)
                if fsize > 0 and fsize < 25 * 1024 * 1024:
                    with open(resolved, "rb") as f:
                        upload_result = await zernio.upload_direct(
                            file_content=f.read(),
                    ...
                    # Presign upload for large files (returns uploadUrl + publicUrl)
                    presign = await zernio.presign_upload(
                        filename=os.path.basename(resolved),
                        content_type="video/mp4",
                        size=fsize,
                    )
                    if "error" not in presign:
                        upload_url = presign.get("uploadUrl", "")
                        public_url = presign.get("publicUrl", "")
                        if upload_url and public_url:
                            async with httpx.AsyncClient() as hx:
                                with open(resolved, "rb") as f:
                                    await hx.put(upload_url, content=f.read())

        if not public_url:
            return self._error(platform, "Could not resolve media URL")

        # 2. Find connected account --------------------------------------------
        accounts = await zernio.list_accounts(platform=platform, status="connected")
        if not accounts:
            return self._error(
                platform,
                f"No connected {platform} account found in Zernio. "
                f"Connect one at https://zernio.com/dashboard",
            )
        ...
        if "error" in result:
            return self._error(platform, f"Zernio post failed: {result['detail']}")

        post = result.get("post", {})
        post_url = ""
        platforms_res = post.get("platforms", [])
        if platforms_res:
            post_url = platforms_res[0].get("platformPostUrl", "")
    ...
    async def upload(
        ...
        Uses **Zernio API** when the API key is configured (preferred).
        Falls back to **BrowserUse** (Playwright) for browser automation.
        ...
        # ── Zernio API path (preferred) ───────────────────────────────
        zernio = await self._get_zernio_client(platform)
        if zernio and zernio.is_authenticated:
            result = await self._upload_via_zernio(
                platform=platform,
                caption=caption,
                hashtags=hashtags,
            ...
            if result.get("status") == "uploaded":
                return result
            # Fall through to browser on failure; log the Zernio error
            zernio_error = result.get("error", "unknown")
            print(f"[AutoUploader] Zernio upload failed, falling back to browser: {zernio_error}")

        # ── BrowserUse fallback path ──────────────────────────────────
